import { useEffect } from "react";
import { useTranslation } from "react-i18next";
import { ScrollView, StyleSheet, Text, View } from "react-native";
import { BarChart, RadarChart } from "react-native-gifted-charts";
import { useSelector } from "react-redux";
import { useWeather } from "../../../server/useWeather";
import type { RootState } from "../../../store/store";
import type { ForecastdayList } from "../../../types";
import { darkTheme, lightTheme } from "../../../utils/theme/theme";

export default function Analytics() {
	const { t } = useTranslation();
	const { darkMode } = useSelector((state: any) => state.theme);
	const { location } = useSelector((state: RootState) => state.location);

	const theme = darkMode ? darkTheme : lightTheme;

	const { weatherData, handlerWeatherData } = useWeather(
		location?.latitude ?? 0,
		location?.longitude ?? 0,
	);

	const precipitationData = weatherData
		? (weatherData as ForecastdayList).map((day, index) => ({
				value: day.day.totalprecip_mm,
				label: new Date(day.date).toLocaleDateString("en-US", {
					weekday: "short",
				}),
				frontColor: theme.color === "#000000" ? "#4A90E2" : "#5DADE2",
			}))
		: [];

	const temperatureData = weatherData
		? (weatherData as ForecastdayList).map((day, index) => ({
				value: day.day.avgtemp_c,
				label: new Date(day.date).toLocaleDateString("en-US", {
					weekday: "short",
				}),
				dataPointText: `${Math.round(day.day.avgtemp_c)}°C`,
			}))
		: [];

	const radarTemperatureData = weatherData
		? (weatherData as ForecastdayList).map((day) => day.day.avgtemp_c)
		: [];

	useEffect(() => {
		if (location?.latitude && location?.longitude) {
			handlerWeatherData(30);
		}
	}, [location, handlerWeatherData]);
	return (
		<ScrollView
			style={{ backgroundColor: theme.backgroundColor }}
			contentContainerStyle={styles.container}
		>
			<Text style={[styles.title, { color: theme.color }]}>
				{t("weatherAnalytics.title")}
			</Text>

			<View style={styles.chartContainer}>
				<Text style={[styles.chartTitle, { color: theme.color }]}>
					{t("weatherAnalytics.charts.precipitation")}
				</Text>
				<BarChart
					data={precipitationData}
					barWidth={40}
					spacing={24}
					barBorderRadius={4}
					yAxisThickness={0}
					xAxisThickness={0}
					yAxisTextStyle={{ color: theme.color }}
					xAxisLabelTextStyle={{ color: theme.color }}
					noOfSections={4}
					maxValue={Math.max(
						...precipitationData.map((item) => item.value),
						10,
					)}
				/>
			</View>

			<View style={styles.chartContainer}>
				<Text style={[styles.chartTitle, { color: theme.color }]}>
					{t("weatherAnalytics.charts.temperature")}
				</Text>

				<RadarChart
					data={radarTemperatureData}
					labels={temperatureData.map((item) => item.label)}
				/>
			</View>
		</ScrollView>
	);
}

const styles = StyleSheet.create({
	container: {
		flex: 1,
		paddingHorizontal: 20,
		paddingTop: 100,
	},
	title: {
		fontSize: 24,
		fontWeight: "bold",
		textAlign: "center",
		marginBottom: 30,
	},
	chartContainer: {
		marginBottom: 40,
		alignItems: "center",
	},
	chartTitle: {
		fontSize: 18,
		fontWeight: "600",
		marginBottom: 20,
		textAlign: "center",
	},
});
